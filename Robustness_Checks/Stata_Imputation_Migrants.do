...
// Bring both variables together to avoid missings
gen edu5 = .
replace edu5 = edu2
...
/*------------------------------------------------------------------------
Number of Coethnic Students
------------------------------------------------------------------------*/ 

// Tunrning ethnic group variables into strings
tostring eth, gen(eth3)
tostring eth2, gen(eth4)

// Count matches of individual ethnicity with others from same ethnic in classroo
countmatch eth3 eth4, gen(coeth1) by(ID_cc)

// Subtracting 1 for respondent
gen coeth = coeth1 - 1
sort ID_cc
...
/************************************************************************
Generating Averages on Class Level for ses, cog, mig and coeth
*************************************************************************/
sort ID_cc
...
gen meanmig2 = meanmig * 100
// Dividing by 10
gen percmig = meanmig2 / 10

// Generating percentage of coethnics
gen meancoeth = coeth / ID_cc_freq
gen meancoeth2 =  meancoeth * 100
// Dividing by 10
gen perccoeth = meancoeth2 / 10
...
keep ID_cc ID_t sat math cog ses edu mig female age attedu lang generation school meanses meancog percmig perccoeth
...
// Dropping observations of ethnic majority
drop if mig == 0
...
// Analyzing missings
mdesc sat math cog ses edu mig female age attedu lang generation school meanses meancog percmig perccoeth

mi misstable summarize sat math cog ses edu female age attedu lang generation school meanses meancog percmig perccoeth


mi misstable patterns at math cog ses edu female age attedu lang generation school meanses meancog percmig perccoeth



// Imputation
mi register imputed sat math cog ses edu female age attedu lang meancog

mi register regular percmig meanses generation school perccoeth

mi impute chained (pmm,knn(5)) ses math sat cog meancog age (ologit) edu attedu (logit) lang female = i.generation i.school percmig meanses perccoeth, burn(10) add(20) rseed(9478)
